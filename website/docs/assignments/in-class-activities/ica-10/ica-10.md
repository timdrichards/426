...
Lecture 10 code is organized under `code/10-idempotency`:
...
- `code/10-idempotency/activity-2`
...
Run from `code/10-idempotency/activity-2`:
...
```bash
cd code/10-idempotency/activity-2 && docker compose down
```
